...
Inc_Done:
	...
	mov a, state
	cjne a, #5, TURN_SSR_OFF
	sjmp CHECK_SSR_ON_OFF
...
CONT:
	...
	TEMPERATURE_CHECKER(#50H, #00H, x_lteq_y, bcd+1)	;if in the first 60s temp <= 50, abort reflow process
	jnb mf, continuee
	clr mf
...
STAGE1_RAMP_TO_SOAK:
	...
	cjne a, #1, STAGE2_PREHEAT_JUMPER
...
;	jb STAGE2_DONE_FLAG, STATE3_RAMP_TO_PEAK_JUMPER
	;cjne a, #0x33, DONE1
	cjne a, #2, STATE3_RAMP_TO_PEAK_JUMPER
	mov pwm, #20
...
STAGE4_REFLOW:
	...
	cjne a, #4, STAGE5_COOLING_JUMPER
	mov pwm, #20
...
STAGE5_COOLING:
	...
	cjne a, #5, forever_jumper4
	mov pwm, #0
	TEMPERATURE_CHECKER (#30H, #0, x_lteq_y, bcd+1) ;if temp <= 30, beep six times
